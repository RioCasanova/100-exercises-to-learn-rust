...
impl Order {
    ...
    fn validate_name(new_name: &String) {
        if new_name.as_bytes().len() > 300 {
            panic!("'Name' must be less than 300 bytes: Make name shorter")
        }
        if new_name.is_empty() {
            panic!("'Name' must not be empty: Please enter a value")
        }
    }

    fn validate_quantity(new_quantity: &u32) {
        if *new_quantity == 0 {
            panic!("'Quantity' must not be empty: Please enter a value")
        }
    }

    fn validate_unit_price(new_unit_price: &u32) {
        if *new_unit_price == 0 {
            panic!("'Unit Price' must not be empty: Please enter a value")
        }
    }
